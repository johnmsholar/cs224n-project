...
class Max_Attentive_Matching_Layer(Attention_Base_Class):
    ...
    def compute_attention(self, a, b, W):
        ...
        def body(i, result):
            h_i = tf.expand_dims(a_perm[i, :, :], axis=0) # 1 x hidden_size x batch
            alpha = self.compute_alpha_embeddings(h_i, b_perm) # batch x B_time_steps

            # get index for each batch of best timestep
            max_indices = tf.argmax(alpha, axis=1) # batch x 1, time_step index
            print ("max_indices", max_indices.get_shape())
            # lookup = tf.one_hot(max_indices, b_time_steps) # batch x b_timesteps
            # print lookup.get_shape()
            h_i_mean = tf.nn.embedding_lookup(b, max_indices) # batch x hidden
            print h_i_mean.get_shape()
            assert False
...
if __name__ == "__main__":
    ...
    with tf.Session() as session:
        ...
        mam = Max_Attentive_Matching_Layer(num_perspectives)
        score_fn = mam.compute_attention(tf.constant(A, dtype=tf.float32), tf.constant(B, dtype=tf.float32), tf.constant(W, dtype=tf.float32))
