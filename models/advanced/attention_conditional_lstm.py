...
"""
CS 224N 2016-2017
attention_conditional_lstm.py: Attention Conditional LSTM Implementation
...
class Attention_Conditional_Encoding_LSTM_Model(Model):
    """
    """
    ...
    def __init__(self, config):
        ...
        self.embedding_matrix = tf.constant(self.config.pretrained_embeddings,
                                            dtype=tf.float32,
    ...
    def add_placeholders(self):
        ...
        """
        # Sequence Lengths
        self.h_sequence_lengths_placeholder = tf.placeholder(tf.int32, (None),
                                                             name="h_seq_lengths")
        self.b_sequence_lengths_placeholder = tf.placeholder(tf.int32, (None),
                                                             name="b_seq_lengths")

        # inputs
        self.headline_placeholder = tf.placeholder(tf.int32, shape=(
        None, self.config.h_max_length), name="headline_input")
        self.body_placeholder = tf.placeholder(tf.int32, shape=(
        None, self.config.b_max_length), name="body_input")

        self.labels_placeholder = tf.placeholder(tf.float32, (
        None, self.config.num_classes), name="labels")
        self.dropout_placeholder = tf.placeholder(tf.float32, name="dropout")

    def create_feed_dict(self, headline_batch, body_batch, labels_batch=None,
                         dropout=1, h_sequence_lengths=[],
                         b_sequence_lengths=[]):
    ...
    def add_embedding(self, isHeadline):
        ...
        """
        if isHeadline:
            e = tf.nn.embedding_lookup(self.embedding_matrix,
                                       self.headline_placeholder)
            embeddings = tf.reshape(e, shape=[-1, self.config.h_max_length,
                                              self.config.embed_size],
                                    name="embeddings_matrix_h")
        else:
            e = tf.nn.embedding_lookup(self.embedding_matrix,
                                       self.body_placeholder)
            embeddings = tf.reshape(e, shape=[-1, self.config.b_max_length,
                                              self.config.embed_size],
                                    name="embeddings_matrix_b")
        end_time = time.time()
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        with tf.variable_scope("headline_cell"):
            ...
            headline_outputs, headline_state = tf.nn.dynamic_rnn(cell_headline, headline_x,
                                                  dtype=tf.float32,
                                                  sequence_length=self.h_sequence_lengths_placeholder)

        # run second LSTM that accept state from first LSTM
        with tf.variable_scope("body_cell"):
            cell_body = tf.contrib.rnn.LSTMBlockCell(
                num_units=self.config.hidden_size)
            body_outputs, _ = tf.nn.dynamic_rnn(cell_body, body_x,
    ...
    def train_on_batch(self, sess, headline_batch, body_batch, labels_batch):
        ...
        feed = self.create_feed_dict(headline_batch, body_batch,
                                     labels_batch=labels_batch,
                                     h_sequence_lengths=h_sequence_lengths,
                                     b_sequence_lengths=b_sequence_lengths,
    ...
    def run_epoch(self, sess, train_examples, dev_set):
        prog = Progbar(
            target=1 + len(train_examples[0]) / self.config.batch_size)
        for i, (headline_batch, article_batch, labels_batch) in enumerate(
                minibatches(train_examples, self.config.batch_size)):
            loss = self.train_on_batch(sess, headline_batch, article_batch,
                                       labels_batch)
        ...
        for i, (headline_batch, article_batch, labels_batch) in enumerate(
            ...
            predictions_batch = list(
                self.predict_on_batch(sess, headline_batch, article_batch))
        ...
        for epoch in range(self.config.n_epochs):
            ...
            if dev_score > best_dev_score:
                ...
                if saver:
                    ...
                    saver.save(sess,
                               './data/weights/conditional_lstm_best_stance.weights')
            if saver:
                print "Finished Epoch ... Saving model in ./data/weights/conditional_lstm_curr_stance.weights"
                saver.save(sess,
                           './data/weights/conditional_lstm_curr_stance.weights')
            print
...
def main(debug=True):
    ...
    with tf.Graph().as_default():
        ...
        model = Attention_Conditional_Encoding_LSTM_Model(config)
        ...
        with tf.Session() as session:
            session.run(init)
            exclude_names = set(["embeddings_matrix:0", "embeddings_matrix_h:0",
            ...
            if not debug:
                ...
                preds = []
                for i, (
                headline_batch, article_batch, labels_batch) in enumerate(
                        minibatches(test_set, config.batch_size)):
                    predictions_batch = list(
                        model.predict_on_batch(sess, headline_batch,
                                               article_batch))
                    preds.extend(predictions_batch)
                test_score = pretty_report_score(actual, preds,
                                                 "./data/plots/conditional_lstm_confusion_matrix.png")
                print "- test Score: {:.2f}".format(test_score)
                print "Writing predictions"
                with open(
                        './data/predictions/conditional_encoding_lstm_predicted.pkl',
                        'w') as f:
